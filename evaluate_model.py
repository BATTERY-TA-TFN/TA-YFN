...
for ii in range(args.itr):
    ...
    # ---------------------------  记录推理计时 ---------------------------
    total_infer_time = 0.0  # 推理总耗时（秒）
    total_infer_samples = 0  # 推理样本总数
    with torch.no_grad():
        for i, (cycle_curve_data, curve_attn_mask, labels, life_class, scaled_life_class, weights, dataset_ids, seen_unseen_ids) in tqdm(enumerate(test_loader)):
            ...
            torch.cuda.synchronize() if torch.cuda.is_available() else None  # 等待 GPU 完成
    ...
    if accelerator.check_trigger():
        ...
        if total_infer_samples > 0:
            accelerator.print(f"平均单样本推理耗时: {total_infer_time / total_infer_samples * 1000:.4f} ms | 总推理耗时: {total_infer_time:.2f} s | 样本数: {total_infer_samples}")
